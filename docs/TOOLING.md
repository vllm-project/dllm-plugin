...
| **Optional vLLM bound** | Optional extra in `pyproject.toml` is **`vllm>=0.14.0,<0.15`**. Keep README and DESIGN_MVP wording aligned; track hook compatibility in [vllm#36155](https://github.com/vllm-project/vllm/issues/36155) and plugin issue [#2](https://github.com/vllm-project/dllm-plugin/issues/2). |
